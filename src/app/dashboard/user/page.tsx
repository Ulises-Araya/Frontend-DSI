...
import { getUserShifts } from '@/lib/actions';
...
import { PlusCircle, CalendarCheck, Archive } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function UserDashboardPage() {
  ...
  const [userDNI, setUserDNI] = useState<string | undefined>(undefined); // To distinguish created vs invited shifts

  async function loadShifts() {
    setIsLoading(true);
    // In a real app, get current user ID from session
    // For mock, actions.ts has a currentUserId
    const fetchedShifts = await getUserShifts();
    // const currentUser = await getCurrentUserMock(); // if needed to get DNI
  ...
  const activeShifts = shifts.filter(s => s.status === 'pending' || s.status === 'accepted');
  const pastOrCancelledShifts = shifts.filter(s => s.status === 'cancelled'); // Or past shifts

  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <h1 className="text-3xl md:text-4xl font-headline text-primary">Mis Turnos</h1>
        <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
          ...
          <DialogContent className="sm:max-w-[480px] bg-card border-primary/30">
            ...
            <CreateShiftForm onShiftCreated={() => { loadShifts(); setIsCreateModalOpen(false); }} setOpen={setIsCreateModalOpen} />
          </DialogContent>
        </Dialog>
      </div>
            ...
            {activeShifts.length > 0 ? (
              <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                {activeShifts.map(shift => (
                  <ShiftCard key={shift.id} shift={shift} currentUserRole="user" onStatusChange={loadShifts} />
                ))}
              </div>
            ) : (
            ...
            <section>
              ...
              <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                {pastOrCancelledShifts.map(shift => (
                  <ShiftCard key={shift.id} shift={shift} currentUserRole="user" />
                ))}
              </div>
            </section>
          )}
